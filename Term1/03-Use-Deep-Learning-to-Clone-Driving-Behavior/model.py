...
def data_loading(delta):
    ...
    loops=tqdm(range(len(logs)))
    for i in loops:
        loops.set_description("Loading No. %s file" %i)
        for j in range(3):
            img_path = logs[i][j]
            img = features_directory + 'IMG' + (img_path.split('IMG')[1]).strip()
...
history = model.fit(train_features, train_labels, batch_size=128, nb_epoch=10, verbose=1,
